...
import { Table, Popconfirm, message, Avatar, Upload, Button, Spin, Modal, Switch} from "antd";
import {
  LinkOutlined,
  DeleteOutlined,
  EditFilled,
...
function AdminDataTable({ data, deleteAccount, isMentee, isPartner, mentors, mentees, refresh}) {
  ...
  useEffect(() => {
    if (data && (data.length !== accounts.length || (data.length > 0 &&data[0].id !== accounts[0].id))) {
      setAccounts(
        data.map((d) => {
  ...
  useEffect(() => {
    if (mentors && (mentors.length !== mentorAccounts.length || (mentors.length > 0 &&mentors[0].id !== mentorAccounts[0].id))) {
      setMentorAccounts(
        mentors.map((d) => {
  ...
  function deleteUser(item, type='mentor'){
    ...
    if (type === 'mentor'){
      ...
      if (selected_partner.assign_mentors && selected_partner.assign_mentors.length > 0){
    ...
    if (type === 'mentee'){
      ...
      if (selected_partner.assign_mentees && selected_partner.assign_mentees.length > 0){
    ...
    if (type === 'mentor'){
      ...
      selectedMentors.map(mentor_id => {
        ...
        if (mentor_record !== null && mentor_record !== undefined){
          selected_partner.assign_mentors.push({id:mentor_id, name:mentor_record.name});
        }
        return false;
      })
      setSelectedPartner(selected_partner);
    ...
    if (type === 'mentee'){
      ...
      selectedMentees.map(mentee_id => {
        ...
        if (mentee_record !== null && mentee_record !== undefined){
          selected_partner.assign_mentees.push({id:mentee_id, name:mentee_record.name});
        }
        return false;
      })
      setSelectedPartner(selected_partner);
  ...
  async function saveData(){
    ...
    await editAccountProfile(edited_data, selectedPartner.id, ACCOUNT_TYPE.PARTNER);
    refresh();
  }
  const AssignUsers = () => {
          ...
          <div className="mentors-area w-50">
            ...
            <div className="flex">
              <ModalInput
                ...
                valid={true}
              />
              <Button disabled={selectedMentors.length > 0 ? false : true} onClick={() => addUsers('mentor')} className="add-btn">+ Add Account</Button>
            </div>
            <div className="list">
              {selectedPartner.assign_mentors && selectedPartner.assign_mentors.map((item, index) => {
                return(
                  <div className={"record " + (index % 2 === 0 ? "even":"odd")}>
                    <div>{item.name}</div>
                    <div style={{cursor:'pointer'}} onClick={() => deleteUser(item, 'mentor')}><DeleteOutlined className="delete-user-btn" /></div>
                  </div>
                )
              })}
            </div>
          ...
          <div className="mentees-area w-50">
            ...
            <div className="flex">
              <ModalInput
                ...
                valid={true}
              />
              <Button disabled={selectedMentees.length > 0 ? false : true} onClick={() => addUsers('mentee')} className="add-btn">+ Add Account</Button>
            </div>
            <div className="list">
              {selectedPartner.assign_mentees && selectedPartner.assign_mentees.map((item, index) => {
                return(
                  <div className={"record " + (index % 2 === 0 ? "even":"odd")}>
                    <div>{item.name}</div>
                    <div style={{cursor:'pointer'}} onClick={() => deleteUser(item, 'mentee')}><DeleteOutlined className="delete-user-btn" /></div>
                  </div>
                )
              })}
            </div>
        ...
        <>
          ...
          <Column
            ...
            render={(mentor_nums, item) => {
              return(
                <span onClick={() => showModal(item)} className='link-span'>{mentor_nums}</span>
              )
            }}
          />
          <Column
            ...
            render={(mentee_nums, item) => {
              return(
                <span onClick={() => showModal(item)} className='link-span'>{mentee_nums}</span>
              )
            }}
          />
          <Column
            ...
            render={(restricted_show, item) => {
              return(
                <span onClick={() => showModal(item)} className='link-span'>{restricted_show}</span>
              )
            }}
          />
          <Column
