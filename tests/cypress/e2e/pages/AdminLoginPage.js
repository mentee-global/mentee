...
export class AdminLogin {
    isFunctional() {
        cy.readFile(translationPath).then((currentLanguage) => {
            ...
            cy.get('#root > section > main > div > div > div.css-1c9mpvn > div > form > div:nth-child(3) > div > div > div > div > a')
    ...
    languageChange() {
        ...
        I18N_LANGUAGES.map((language, index) => {
            ...
            cy.readFile(translationPath).then((currentLanguage) => {
                ...
                // Checking The Text
                cy.get('#root > section > main > div > div > div.css-1c9mpvn > div > h2')
                    .should('contain.text', currentLanguage.common.admin)
                cy.get('#root > section > main > div > div > div.css-1c9mpvn > div > form > div:nth-child(1) > div > div.ant-col.ant-form-item-label.css-wxm1m1 > label')
                    .should('have.attr', 'title', currentLanguage.common.email)
                cy.get('#root > section > main > div > div > div.css-1c9mpvn > div > form > div:nth-child(2) > div > div.ant-col.ant-form-item-label.css-wxm1m1 > label')
    ...
    invalidCredentials() {
        cy.readFile(translationPath).then((currentLanguage) => {
            ...
            cy.get('div > div > div > span:nth-child(2)')
                .should('contain.text', currentLanguage.loginErrors.incorrectCredentials)
        })
    }
}