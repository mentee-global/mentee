describe('Registration for mentor', () => {
  ...
  it('check the validaions for Mentor are they working', () => {
    ...
    cy.get('#firstName_help > div').should('have.text', 'Please enter First Name');

    cy.get('#lastName_help > div').should('be.visible');
    cy.get('#lastName_help > div').should('have.text', 'Please enter Last Name');

    cy.get('#hearAboutUs_help > div').should('be.visible');
    cy.get('#hearAboutUs_help > div').should('have.text', 'Please enter From whom or where did you hear about us?');

    cy.get('#knowledgeLocation_help > div').should('be.visible');
    cy.get('#knowledgeLocation_help > div').should('have.text', 'Please enter Please share which region(s), country(s), state(s), cities your knowledge is based in');

    cy.get('#previousLocations_help > div').should('be.visible');
    cy.get('#previousLocations_help > div').should('have.text', 'Please enter Where have you lived in your life besides where you live now?');

    cy.get('#employerName_help > div').should('be.visible');
    cy.get('#employerName_help > div').should('have.text', 'Please enter Full name of your company/employer');

    cy.get('#jobDescription_help > div').should('be.visible');
    cy.get('#jobDescription_help > div').should('have.text', 'Please enter Your full title and a brief description of your role');

    cy.get('#jobDuration_help > div').should('be.visible');
    cy.get('#jobDuration_help > div').should('have.text', 'Please enter How long have you been with this company?');

    cy.get('#commitDuration_help > div').should('be.visible');
    cy.get('#commitDuration_help > div').should('have.text', 'Please enter If you are accepted as a global mentor, would you like to commit to...');

    cy.get('#immigrationStatus_help > div').should('be.visible');
    cy.get('#immigrationStatus_help > div').should('have.text', 'Please enter Are you an immigrant or refugee or do you come from an immigrant family or refugee family?');

    cy.get('#communityStatus_help > div').should('be.visible');
    cy.get('#communityStatus_help > div').should('have.text', 'Please enter Are you or your family from a native or aboriginal community?');

    cy.get('#economicBackground_help > div').should('be.visible');
    cy.get('#economicBackground_help > div').should('have.text', 'Please enter Did you grow up economically challenged?');

    cy.get('#isPersonOfColor_help > div').should('be.visible');
    cy.get('#isPersonOfColor_help > div').should('have.text', 'Please enter Would you consider yourself of person of color');

    cy.get('#genderIdentification_help > div').should('be.visible');
    cy.get('#genderIdentification_help > div').should('have.text', 'Please enter What do you identify as?');

    cy.get('#isMarginalized_help > div').should('be.visible');
    cy.get('#isMarginalized_help > div').should('have.text', 'Please enter Would you define yourself as having been or currently marginalized');

    cy.get('#languageBackground_help > div').should('be.visible');
    cy.get('#languageBackground_help > div').should('have.text', 'Please enter Do you speak a language(s) other than English? If yes, please write the language(s) below and include your fluency level (conversational, fluent, native)');

    cy.get('#referral_help > div').should('be.visible');
    cy.get('#referral_help > div').should('have.text', 'Please enter If you know someone who would be a great global mentor, please share their name, email, and we\'ll contact them!');

    cy.get('#canDonate_help > div').should('be.visible');
    cy.get('#canDonate_help > div').should('have.text', 'Please enter MENTEE is a volunteer organization and we are sustained by donations. Are you able to offer a donation for one year?');
