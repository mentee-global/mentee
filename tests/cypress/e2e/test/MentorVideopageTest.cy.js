...
describe("chceking the video page of mentor", () => {
  it("checking page elements visibility", () => {
    ...
    cy.get('button.ant-btn-primary span:contains("Add Video")')
      .should(
        "be.visible"
      );
    ...
    cy.get("#video-submit_title")
      .type("my skills intro");

    cy.get("#video-submit_url").type(
      "https://www.youtube.com/watch?v=Gjnup-PuquQ"
    );

    cy.get("#video-submit_tag")
      .click();

    cy.contains(".ant-select-item-option-content", "Computer Science")
      .click();

    cy.get('button.ant-btn-primary span:contains("Submit")')
      .click();


    cy.get('.ant-table-tbody')
      .should('contain.text', 'my skills intro');

    cy.get('#root > section > main > div > div > div > div > div > div > div > table > tbody > tr:nth-child(1) > td:nth-child(3) > div > div:nth-child(2) > button')
      .click();

    cy.contains("span", "Yes")
      .click();
